...
export const userMiddleware = async (req: RequestAuthentication | any, res: Response, next: NextFunction): Promise<void> => {
     ...
     if (token) {
          ...
          jwt.verify(token, TOKEN_SECRET_KEY, async (err: any, decoded: any) => {
               ...
               const user: responseData | any = await UserModel.findOne({ _id: decoded._id, status: "active" }, { _id: 1, tokenVersion: 1 });
